...
export default function SchoolOrderHistoryPage({ params }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title="Order History"
        description="All orders placed through this school's albums."
        actions={
          <Button variant="outline" onClick={() => exportCsv.mutate({ schoolId, search: search || undefined, status: status === "__all__" ? undefined : (status as OrderStatus) })} disabled={exportCsv.isPending}>
            ...
            <TableBody>
              {data.data.map((order) => (
                <TableRow key={order.id}>
                  ...
                  <TableCell className="text-right text-sm text-muted-foreground">{new Date(order.placedAt).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
